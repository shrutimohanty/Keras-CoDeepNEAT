...
class Individual:
    ...
    def fit(self, input_x, input_y, training_epochs=1, validation_split=0.15, current_generation="", custom_fit_args=None):
        ...
        if custom_fit_args is not None:
            ...
            fitness = self.model.fit(input_x, input_y, verbose=2, epochs=training_epochs, validation_split=validation_split, batch_size=128)
...
for individual in self.individuals:
    ...
    self.global_trained_model += 1
    if (self.datasets.custom_fit_args is not None):
        print("self.datasets.custom_fit_args is not None")
        ...
        print("self.datasets.custom_fit_args is None")
...
def func(individual, custom_fit_args, input_x, input_y, test_x, test_y, training_epochs, validation_split, current_generation, thread_id, iteration):
    if (custom_fit_args is not None):
        ...
        history = individual.fit(input_x, input_y, training_epochs, validation_split, current_generation=current_generation)
    score = individual.score(test_x, test_y)
    iteration[thread_id] = [individual.name, individual.blueprint.mark, score, individual.blueprint.get_kmeans_representation(),
                            (None if individual.blueprint.species == None else individual.blueprint.species.name),
                            current_generation]
...
class Population:
    ...
    def iterate_fitness(self, training_epochs=1, validation_split=0.15, current_generation=0, thread_num=None):
        ...
        """
        conf = tf.ConfigProto(device_count={'CPU': 8}, intra_op_parallelism_threads=4, inter_op_parallelism_threads=4)
        ...
        if thread_num is None or thread_num == 1:
            ...
            if len(self.individuals) <= thread_num:
                p_list = []
                for individual in self.individuals:
                    l.append([])
                ...
                for individual in self.individuals:
                    ...
                    #    break
                    if thread_id % thread_num == 0 or thread_id == len(self.individuals):
                        for p in p_list:
                            p.join()
                '''
            return l
